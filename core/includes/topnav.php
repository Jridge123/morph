...
if(file_exists($inc_topnav)) { include_once($inc_topnav); } else { ?>
	...
	<?php if ($morph->topnav_position !== 'topnav_inside' ) { ?>
		<div id="topnav" class="block<?php if($morph->topnav_position == 'topnav_inside'){ echo ' masthead-nav '; } ?><?php echo $morph->topnav_blockfx.' '; echo pt_classes(array('subtext' => $morph->subtext, 'topnav_actionlink' => $topnav_actionlink, 'topdrop' => $morph->topdrop, 'topfish' => $morph->topfish), $morph->site_width); ?><?php if ( $morph->topnav_wrap == 1 && $morph->topnav_position !== 'topnav_inside' ) { echo ' wrap-on'; } else { echo ' wrap-off'; } ?> primary-nav">	
