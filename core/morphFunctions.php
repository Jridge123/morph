...
$css_themelet				= $absolutepath.'/css/themelet.css';
$css_typo					= $absolutepath.'/css/typo.css';
$css_modfx					= $absolutepath.'/css/modfx.css';
...
//	if ( file_exists($css_iphone)) { $document->addStyleSheet($css_iphone); } else { $document->addStyleSheet($templatepath .'/core/css/jqtouch.css'); }	
	...
	//if (!$pack_css) {
		...
		if (($lcbrowser == 'chrome' || $lcbrowser == 'safari' || $browser->getBrowser() == MBrowser::PLATFORM_IPHONE) && file_exists($css_webkit)) $MORPH->addStyleSheetAfter($themeletpath .'/css/webkit.css');
...
function blocks($position, $glob, $jj_const, $classes, $site_width, $debug_modules, $nojs=''){
	...
	global $mainframe;
	$morph = Morph::getInstance();
	$modcount = count(JModuleHelper::getModules($position));
